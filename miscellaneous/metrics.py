...
def RMSE(x_pred: torch.Tensor, x_true: torch.Tensor) -> float:
    ...
    """
    return torch.sqrt(
        torch.mean(torch.square(x_pred.flatten() - x_true.flatten()))
    ).item()
...
# Utility function
####################################################################
def get_metrics(x_true, x_generated):
    r"""
    Given two tensors x_true and x_generated, computes the RMSE, PSNR and SSIM
    between the two, after normalizing them into [0, 1] with linear normalization.
    """
    # Normalization
    x_true = utilities.normalize(x_true.detach().cpu())
    x_generated = utilities.normalize(x_generated.detach().cpu())

    RMSE_value = RMSE(x_true, x_generated)
    PSNR_value = PSNR(x_true, x_generated)
    SSIM_value = SSIM(x_true, x_generated)
    return RMSE_value, PSNR_value, SSIM_value
